...
import { ReactComponent as Svg1 } from '../../img/three_svg.svg';
import { ReactComponent as Svg2 } from '../../img/x2.svg';
...
const TestMath = () => {
    ...
    const [currentSvgIndex, setCurrentSvgIndex] = useState(<Svg1 />);
    ...
    const [isCorrect, setIsCorrect] = useState("checkbox3-test3");
    const test = {
        one: "Вкажіть парну функцію:",
        two: "Вкажіть функцію з рисунку:",
        three: "Яка з наведених графіків є",
        ...
        for (const test in selectedOptions) {
            ...
            if (isCorrect2) {
                ...
                anime({
                    ...
                    loop: false,
                    complete: (anim) => {
                        // Код, що виконується після завершення анімації
                        document.querySelectorAll('.test2_path').forEach(path => {
                            path.setAttribute('fill', 'black'); // Застосувати fill після анімації
            ...
            if (isCorrect3) {
                anime({
                    ...
                    easing: 'easeInOutQuad',
                    complete: () => {
                        setCurrentSvgIndex(<Svg2 />);
                        setIsCorrect("checkbox1-test3")
                        anime({
                            targets: svgContainerRef.current,
                            opacity: 1,
                            duration: 500,
    ...
    return (
        <div ref={container} className={s.test_box}>
            ...
            <div ref={test2Ref} className={classNames(s.two_test, s.test2, {
                ...
                <div className={s.img_test}>
                    <div className={s.img_graf}>
                        <svg className='graph-svg' width="347" height="376" viewBox="0 0 347 376" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path className="test2_path" d="M347 150.4C347 151.196 346.239 151.855 345.321 151.855H175.179V299.344C175.179 300.14 174.418 300.8 173.5 300.8C172.582 300.8 171.821 300.14 171.821 299.344V151.855H1.67903C0.761161 151.855 0 151.196 0 150.4C0 149.604 0.761161 148.944 1.67903 148.944H171.821V1.45548C171.821 0.659819 172.56 0 173.5 0C174.44 0 175.179 0.659819 175.179 1.45548V148.944H345.321C346.239 148.944 347 149.604 347 150.4ZM335.829 144.733C310.8 144.733 231.706 142.87 204.349 119.893C178 97.7697 177.754 10.7706 177.776 7.08335C177.776 5.47262 179.276 4.17239 181.134 4.17239C182.992 4.17239 184.492 5.47262 184.492 7.08335C184.492 7.95664 184.716 95.3051 209.028 115.721C239.005 140.871 342.187 138.873 343.217 138.853C345.097 138.737 346.619 140.076 346.664 141.686C346.709 143.297 345.254 144.636 343.396 144.675C342.769 144.675 340.082 144.753 335.851 144.753L335.829 144.733ZM11.9547 156.067C36.9835 156.067 116.077 157.93 143.434 180.907C169.784 203.03 170.03 290.029 170.008 293.717C170.008 295.327 168.508 296.628 166.65 296.628C164.791 296.628 163.291 295.327 163.291 293.717C163.291 292.843 163.068 205.495 138.755 185.079C108.779 159.929 5.59677 161.927 4.56697 161.947C2.68645 162.063 1.16413 160.724 1.11935 159.113C1.07458 157.503 2.52974 156.164 4.38787 156.125C5.01471 156.125 7.70116 156.047 11.9323 156.047L11.9547 156.067Z" fill="none" stroke="black" />
                        </svg></div>
            ...
            <div ref={test3Ref} className={classNames(s.three_test, s.test3, {
                ...
                <div className={s.qw_three} ref={svgContainerRef} style={{ opacity: 1 }}>
